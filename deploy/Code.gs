...
function saveProduct(data) {
  if (data.id) {
    // Bersihkan file lama di Google Drive jika foto diganti/dihapus
    try {
      const oldProd = DB.getById(CONFIG.SHEET_NAMES.PRODUCTS, data.id);
      if (oldProd && oldProd.photo && oldProd.photo !== data.photo) {
        deleteDriveFileByUrl(oldProd.photo);
      }
    } catch(err) {
      Logger.log('Gagal membersihkan foto lama: ' + err.toString());
    }
...
function deleteProduct(id) {
  try {
    const prod = DB.getById(CONFIG.SHEET_NAMES.PRODUCTS, id);
    if (prod && prod.photo) {
      deleteDriveFileByUrl(prod.photo);
    }
  } catch(err) {
    Logger.log('Gagal menghapus foto produk dari Drive: ' + err.toString());
  }
  DB.delete(CONFIG.SHEET_NAMES.PRODUCTS, id);
  return { success: true };
}

// Fungsi helper untuk menghapus file dari Google Drive berdasarkan URL
function deleteDriveFileByUrl(url) {
  try {
    ...
    Logger.log("Gagal menghapus file Drive: " + e.toString());
  }
}

// Fungsi untuk mengupload foto produk ke Google Drive
function uploadProductPhoto(data) {
  try {
    ...
    // Cari atau buat folder "AplikasiKasir_Photos"
    let folder;
    const folders = DriveApp.getFoldersByName("AplikasiKasir_Photos");
    if (folders.hasNext()) {
      folder = folders.next();
    } else {
      folder = DriveApp.createFolder("AplikasiKasir_Photos");
    }
    
    // Simpan file di folder tersebut
    const file = folder.createFile(blob);
    
    // Atur izin file agar bisa dilihat oleh siapa saja yang memiliki link
    file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
    
    // URL langsung (hotlink) untuk menampilkan gambar di tag img
    const url = "https://drive.google.com/uc?export=view&id=" + file.getId();
    
    return { success: true, url: url, fileId: file.getId() };
  } catch (e) {
    Logger.log("uploadProductPhoto error: " + e.toString());
